...
def do_generate_test(encoder, decoder, attention_decoder, enc_w_list, word_manager, form_manager, opt, using_gpu):
...
main_arg_parser.add_argument('-output', type=str, default='output/seq2seq_transformer_output.txt', help='input data filename')
main_arg_parser.add_argument('-model', type=str, default='checkpoint_dir/model_transformer', help='model checkpoint to use for sampling')
...
with open(args.output, "w") as output:
    ...
    for i in range(len(data)):
        ...
        x = data[i]
        reference = x[1]
        candidate = do_generate_test(encoder, decoder, attention_decoder, x[0], word_manager, form_manager, args, using_gpu)
